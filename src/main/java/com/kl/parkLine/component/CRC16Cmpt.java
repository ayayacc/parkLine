...
public class CRC16Cmpt
{
    ...
    public List<Byte> calcCrc16(List<Byte> data)
    {
        return calcCrc16(data, 0, data.size(), 0xffff);
    }
     ...
     * @return CRC16校验值
     */
    public List<Byte> calcCrc16(List<Byte> data, int offset, int len, int preval) {
        int ucCRCHi = (preval & 0xff00) >> 8;
        int ucCRCLo = preval & 0x00ff;
        int iIndex;
        for (int i = 0; i < len; ++i) 
        {
            iIndex = (ucCRCLo ^ data.get(offset + i)) & 0x00ff;
            ucCRCLo = ucCRCHi ^ crc16_tab_h[iIndex];
            ucCRCHi = crc16_tab_l[iIndex];
        }
        List<Byte> bytes = new ArrayList<Byte>();
        bytes.add((byte) ((ucCRCLo & 0x00ff)));
        bytes.add((byte) ((ucCRCHi & 0x00ff)));
        return bytes;
    }
}
